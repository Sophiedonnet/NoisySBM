                      ...
                      verbose = interactive(), ...)
{
  ...
  if (is.null(x)) {
    ...
    if (!is.null(prior)) {
      modelNames <- setdiff(modelNames, c("EVE",
                                          "VEE", "VVE", "EVV"))
    }
    if (is.null(G)) {
  ...
  if (is.null(initialization$noise)) {
    ...
    if (is.null(initialization$subset)) {
      if (is.null(initialization$hcPairs)) {
        if (d != 1) {
          if (n > d) {
            hcPairs <- hc(data = data, modelName = mclust.options("hcModelName")[1])
          }
          else {
